...
impl Formatter for DatetimeFormatter {
    fn format(&self, val: &Value, _config: &SharedConfig) -> Result<String, FormatError> {
        match val {
            Value::Datetime(datetime, timezone) => Ok(match self {
                DatetimeFormatter::Chrono { items, locale } => match *locale {
                    ...
                    None => match timezone {
                        Some(tz) => datetime.with_timezone(tz).format_with_items(items.iter()),
                        None => datetime
                            .with_timezone(&Local)
                ...
                DatetimeFormatter::Icu { locale, length } => {
                    ...
                    dft.format_to_string(&date).map_err(|_| {
                        FormatError::Other(Error::new("formatting date using icu failed"))
                    })?
                }
            }),
            other => Err(FormatError::IncompatibleFormatter {
